...
def main(validate: bool = False, test_model: bool = False):
    ...
    try:
        ...
        for (image, path), label in ds:
            title = "Labeled {}".format(untrained_dataset.class_names[tf.argmax(tf.cast(label, tf.int32))])
            if deep_model:
                expected = deep_model.predict(path.numpy().decode("utf-8"), "people_model")
                expected_label = expected['label']
                title += " Expected {} {}".format(expected_label,
